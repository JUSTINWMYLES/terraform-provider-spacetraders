...
func (r *TransferCargoAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Transfer cargo between ships.\n\nThe receiving ship must be in the same waypoint as the transferring ship, and it must able to hold the additional cargo after the transfer is complete. Both ships also must be in the same state, either both are docked or both are orbiting.\n\nThe response body's cargo shows the cargo of the transferring ship after the transfer is complete.", Attributes: map[string]schema.Attribute{"body_ship_symbol": schema.StringAttribute{MarkdownDescription: "The symbol of the ship to transfer to.", Required: true}, "ship_symbol": schema.StringAttribute{MarkdownDescription: "The symbol of the ship.", Required: true}, "trade_symbol": schema.StringAttribute{MarkdownDescription: "The good's symbol.", Required: true}, "units": schema.Int64Attribute{MarkdownDescription: "Amount of units to transfer.", Required: true}}}
}

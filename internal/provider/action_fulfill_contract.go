...
func (r *FulfillContractAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Fulfill a contract. Can only be used on contracts that have all of their delivery terms fulfilled.", Attributes: map[string]schema.Attribute{"contract_id": schema.StringAttribute{MarkdownDescription: "The ID of the contract to fulfill.", Required: true}}}
}

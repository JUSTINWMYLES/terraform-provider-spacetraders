...
func (r *ExtractResourcesWithSurveyAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Use a survey when extracting resources from a waypoint. This endpoint requires a survey as the payload, which allows your ship to extract specific yields.\n\nSend the full survey object as the payload which will be validated according to the signature. If the signature is invalid, or any properties of the survey are changed, the request will fail.", Attributes: map[string]schema.Attribute{"deposits": schema.DynamicAttribute{MarkdownDescription: "A list of deposits that can be found at this location. A ship will extract one of these deposits when using this survey in an extraction request. If multiple deposits of the same type are present, the chance of extracting that deposit is increased.", Required: true}, "expiration": schema.StringAttribute{MarkdownDescription: "The date and time when the survey expires. After this date and time, the survey will no longer be available for extraction.", Required: true}, "ship_symbol": schema.StringAttribute{MarkdownDescription: "The symbol of the ship.", Required: true}, "signature": schema.StringAttribute{MarkdownDescription: "A unique signature for the location of this survey. This signature is verified when attempting an extraction using this survey.", Required: true}, "size": schema.StringAttribute{MarkdownDescription: "The size of the deposit. This value indicates how much can be extracted from the survey before it is exhausted.", Required: true}, "symbol": schema.StringAttribute{MarkdownDescription: "The symbol of the waypoint that this survey is for.", Required: true}}}
}

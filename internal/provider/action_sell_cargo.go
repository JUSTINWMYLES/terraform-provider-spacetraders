...
func (r *SellCargoAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Sell cargo in your ship to a market that trades this cargo. The ship must be docked in a waypoint that has the `Marketplace` trait in order to use this function.", Attributes: map[string]schema.Attribute{"ship_symbol": schema.StringAttribute{MarkdownDescription: "The symbol of the ship.", Required: true}, "symbol": schema.StringAttribute{MarkdownDescription: "The good's symbol.", Required: true}, "units": schema.Int64Attribute{MarkdownDescription: "Amounts of units to sell of the selected good.", Required: true}}}
}
